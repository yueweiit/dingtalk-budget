...
-- 审批通过后生效的预算额度台账。预算申请主表仍保存全部审批状态。
CREATE TABLE "public"."budget_effective_quota" (
...
CREATE TABLE "public"."budget_effective_quota_subject" (
...
CREATE INDEX "idx_budget_effective_quota_period_company_dept"
  ON "public"."budget_effective_quota" ("budget_month", "company_dept_id", "department_id")
  WHERE "status" = 'effective';
CREATE INDEX "idx_budget_effective_quota_period_group_company_dept"
  ON "public"."budget_effective_quota" ("budget_month", "group_dept_id", "company_dept_id", "department_id")
  WHERE "status" = 'effective';
CREATE INDEX "idx_budget_effective_quota_subject_code"
