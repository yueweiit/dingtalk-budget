...
When no eligible payment event exists, a non-split expense falls back to the completed-and-agreed approval amount and completion time. Salary, reserve fund, social insurance, housing fund, office space, and individual income tax forms continue to use department split rows and do not use whole-form payment events, preventing double counting. Reserve-fund rows are only produced when the configured process selects `备用金` in the management-expense component and has a non-empty reserve-fund detail table. Historical `bonus` storage keys remain readable.
