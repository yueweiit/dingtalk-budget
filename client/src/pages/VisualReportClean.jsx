...
const RegionTooltip = ({ active, payload }) => {
  if (!active || !payload || !payload.length) return null;
  const row = payload.find((item) => Number(item.value) > 0) || payload[0];
  if (!row) return null;
  ...
  return (
    <div style={{
      ...
      <p style={{ margin: '0 0 4px', fontWeight: 600, color: '#374151' }}>{row.name}</p>
      <p style={{ margin: 0, color: row.color }}>{formatCurrency(row.value)}</p>
    </div>
  );
};

const RegionBarLabel = ({ x, y, width, height, value, fill, payload }) => {
  if (!value) return null;
  const label = value === payload.production ? '预算' : '支出';
...
export default function VisualReportClean({ onBack }) {
  ...
  if (errorMessage) {
    return <div style={styles.empty}>{errorMessage}</div>;
  }

  if (!chartData) {
    return <div style={styles.empty}>暂无报表数据</div>;
  }
        ...
        <div style={styles.chartGrid}>
          ...
          <div style={styles.chartCard}>
            ...
            <ResponsiveContainer width="100%" height={Math.max(240, regionDist.length * 58)}>
              <BarChart data={regionDist} layout="vertical" margin={{ top: 5, right: 40, left: 110, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f3f4f6" />
